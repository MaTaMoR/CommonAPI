...
public abstract class SimpleSingleSQLDatabaseManager<K, V> extends SimpleSQLDatabaseManager implements SingleDatabaseManager<K, V> {

    public SimpleSingleSQLDatabaseManager(Plugin plugin, String name, double queriesVersion) {
        super(plugin, name, queriesVersion);
    }

    public SimpleSingleSQLDatabaseManager(Plugin plugin, String name, double queriesVersion, boolean open) {
    ...
    public V load(K key) {
        try {
            ...
            throw new DatabaseException("[Database: " + getName() + "] Couldn't loadOrCreate data from: " + key.toString(), e);
        }
    }
