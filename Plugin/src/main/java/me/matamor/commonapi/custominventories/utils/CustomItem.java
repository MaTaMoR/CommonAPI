...
import me.matamor.commonapi.custominventories.utils.amount.Amount;
import me.matamor.commonapi.custominventories.utils.amount.AmountUtil;
import me.matamor.commonapi.custominventories.utils.amount.SimpleAmount;
import me.matamor.commonapi.custominventories.utils.server.ServerVariables;
...
public class CustomItem implements Cloneable, ConfigurationSerializable {
    ...
    private final Map<String, InventoryVariable> localVariables = new HashMap<>();
    ...
    public void setSkullOwner(String skullOwner) {
        ...
        this.skullData = new SkullData(skullOwner, SkullData.SkullDataType.NAME);
    }

    public void setSkullOwner(String name, UUID uuid) {
        Validate.notNull(name, "Name can't be null");
        Validate.notNull(uuid, "UUID can't be null");

        this.skullData = new SkullData(SkullData.SkullDataType.OFFLINE_PLAYER, name + ":" + uuid.toString());
    }
    ...
    protected String calculateName(Player player) {
        if (hasName()) {
            ...
            name = ServerVariables.replace(name);
            if (player != null) {
                for (InventoryVariable inventoryVariable : getLocalVariables()) {
                    name = name.replace(inventoryVariable.getVariable(), inventoryVariable.getReplacement(player));
                }

                name = InventoryVariables.replace(name, player);
            }

            if (hasCalculateEvents()) {
                for (CalculateEvent event : calculateEvents) {
                    name = event.calculate(CalculateEvent.CalculateType.NAME, name);
                }
            }
    ...
    protected List<String> calculateLore(Player player) {
        ...
        if (hasLore()) {
            for (String line : this.lore) {
                lore.add(ServerVariables.replace(line));
            }

            if (player != null) {
                for (String line : lore) {
                    for (InventoryVariable lineInventoryVariable : getLocalVariables()) {
                        line = line.replace(lineInventoryVariable.getVariable(), lineInventoryVariable.getReplacement(player));
                    }

                    line = InventoryVariables.replace(line, player);

                    if (hasCalculateEvents()) {
                        for (CalculateEvent event : calculateEvents) {
                            line = event.calculate(CalculateEvent.CalculateType.LORE, line);
                        }
                    }
    ...
    public ItemStack build(Player player) {
        ...
        if (hasName()) itemMeta.setDisplayName(InventoryUtils.color(calculateName(player)));

        if (hasLore()) itemMeta.setLore(InventoryUtils.color(calculateLore(player)));
    ...
    public CustomItem clone() {
        CustomItem cloned = new CustomItem(getMaterial(), getAmount(), getDataValue());
    ...
    public static class Builder {
        ...
        public Builder setSkullOwner(String name, UUID uuid) {
    ...
    public static Map<String, Object> serialize(CustomItem customItem) {
        ...
        if (customItem.hasColor()) serialized.put(Node.COLOR, customItem.getColor().serialize());
        ...
        if (customItem.hasFireworkEffect()) serialized.put(Node.FIREWORK_EFFECT, customItem.getFireworkEffect().serialize());
        ...
        if (customItem.hasItemFlags()) serialized.put(Node.ITEM_FLAGS, serializeItemFlags(customItem.getItemFlags()));
    ...
    public static CustomItem deserialize(ConfigurationSection section) {
        ...
        if (section.isSet(Node.SKULL_OWNER)) {
            Map<String, Object> map = InventoryUtils.asMap(section.get(Node.SKULL_OWNER));

            if (map.containsKey("Value") && map.containsKey("Type")) {
                skullData = new SkullData((String) map.get("Value"), SkullData.SkullDataType.getByName((String) map.get("Type")));
            }
        }
        ...
        if (section.isSet(Node.FIREWORK_EFFECT)) {
            Map<String, Object> map = InventoryUtils.asMap(section.get(Node.FIREWORK_EFFECT));
