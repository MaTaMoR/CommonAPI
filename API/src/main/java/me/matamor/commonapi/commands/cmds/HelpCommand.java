...
public class HelpCommand<T extends Plugin> extends ICommand<T> {

    public HelpCommand(ICommand<T> parent) {
        super(parent.getPlugin(), "help", "help command", parent, new String[] { });
    }

    @Override
    public void onCommand(CommandArgs commandArgs) {
        Collection<ICommand<T>> children = getParent().getChildren();
        ...
        if (children.isEmpty()) {
            ...
            children.forEach(c -> commandArgs.sendMessage("&a%s &8- &f%s", commandArgs.getLabel(), c.getDescription()));
        }
    }
}
